 ...
 */
function formatAlert({ properties }) {
...
server.tool("get-forecast", "Get weather forecast for a location", {
    ...
    return {
        content: [{
                type: "text",
                text: `🌤 Forecast for (${latitude}, ${longitude}):\n\n${forecastData.properties.periods.map((p) => `📅 ${p.name}: ${p.temperature}°${p.temperatureUnit}, ${p.shortForecast}, 🌬️ ${p.windSpeed} ${p.windDirection}`).join("\n")}`,
            }],
    };
});
/**
 ...
 */
server.tool("get-stock-price", "Fetch the current price of a stock", {
 ...
 */
server.tool("get-historical-data", "Fetch historical stock data", {
    symbol: z.string(),
    startDate: z.string(),
    endDate: z.string(),
    ...
    try {
        ...
        if (!result.length)
            return { content: [{ type: "text", text: `🚫 No historical data for ${symbol} in this range.` }] };
 ...
 * Fetch stock news
 */
server.tool("get-news", "Fetch stock-related news", {
    stockName: z.string(),
    startDate: z.string(),
    endDate: z.string(),
    ...
    try {
        ...
        return {
            content: [{
                    type: "text",
                    text: `📰 Top News for ${stockName}:\n\n${response.articles.slice(0, 5).map((a) => `📌 ${a.title}\n🔗 ${a.url}`).join("\n\n")}`,
                }],
        };
    }
    catch (error) {
 ...
 */
server.tool("get-technical-indicators", "Calculate technical indicators for a stock", {
    symbol: z.string().describe("Stock ticker symbol (e.g., AAPL, TSLA)"),
    indicator: z.enum([
        "sma",
        "ema",
        ...
        switch (indicator) {
            ...
            case "rsi": {
                ...
                result = `🔍 RSI(${period}) for ${symbol}:\n\n${rsiResults.slice(-10).map(day => {
                    let rsiLevel = "";
                    if (day.rsi > 70)
                        rsiLevel = "⚠️ Potentially Overbought";
                    else if (day.rsi < 30)
                        rsiLevel = "⚠️ Potentially Oversold";
            ...
            case "bollinger": {
                ...
                result = `🎯 Bollinger Bands(${period}, ${standardDeviation}σ) for ${symbol}:\n\n${bbandsResults.slice(-10).map(day => {
                    let position = "";
                    if (day.price > day.upper)
                        position = "⚠️ Above Upper Band";
                    else if (day.price < day.lower)
                        position = "⚠️ Below Lower Band";
 ...
 */
server.tool("get-technical-analysis", "Get comprehensive technical analysis for a stock", {
    symbol: z.string().describe("Stock ticker symbol (e.g., AAPL, TSLA)"),
    startDate: z.string().describe("Start date in YYYY-MM-DD format"),
    endDate: z.string().describe("End date in YYYY-MM-DD format"),
